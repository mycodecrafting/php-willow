...
class Willow_Loader
{
    ...
    public static function buildPath($dataPath, $ext = false)
    {
        ...
        // special case for Vendor:Willow (load from core dir)
        if ($dataPath[0] === 'Vendor' && $dataPath[1] === 'Willow')
        {
            $path = Willow::getCoreDir() . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, array_slice($dataPath, 2));
        }
        else
        {
